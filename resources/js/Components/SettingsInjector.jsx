...
import { applyCssVars, upsertMeta, upsertLink, injectScript, injectHtml } from "../lib/domUtils";
...
function applySeo(settings) {
  ...
  if (seo.og_description) upsertMeta("property", "og:description", seo.og_description);
  if (seo.og_image) upsertMeta("property", "og:image", seo.og_image);
  if (seo.og_site_name) upsertMeta("property", "og:site_name", seo.og_site_name);
...
function applyAnalytics(settings) {
  ...
  // Google Tag (gtag)
  if (a.google_tag_id) {
    injectScript({ id: "gtm-src", src: `https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(a.google_tag_id)}`, attrs: { async: "" } });
  ...
  if (a.matomo_url && a.matomo_site_id) {
    const code = `var _paq=window._paq=window._paq||[];_paq.push(['trackPageView']);_paq.push(['enableLinkTracking']);(function(){var u='${a.matomo_url.replace(/\/$/, '/') }';_paq.push(['setTrackerUrl', u+'matomo.php']);_paq.push(['setSiteId', '${a.matomo_site_id}']);var d=document,g=d.createElement('script'),s=d.getElementsByTagName('script')[0];g.async=true;g.src=u+'matomo.js';s.parentNode.insertBefore(g,s);})();`;
...
function applyPerformance(settings) {
  ...
  (p.preconnect_hosts || []).forEach((h) => upsertLink("preconnect", h, { crossorigin: "anonymous" }));
  (p.preload_fonts || []).forEach((f) => upsertLink("preload", f, { as: "font", crossorigin: "anonymous" }));
  (p.preload_scripts || []).forEach((s) => upsertLink("preload", s, { as: "script" }));
}
