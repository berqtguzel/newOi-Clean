...
export default function HeroSection({ content = {} }) {
    ...
    const { sliders, loading: slidersLoading } = useSliders({ tenantId, locale });
    const primarySlide = sliders && sliders.length ? sliders[0] : null;

    const heroTitleHtml = primarySlide?.title || t("hero.title", "İhtiyaçlarınızı karşılayan güvenilir ortağınız");
    const heroSubtitleHtml = primarySlide?.description || t("hero.subtitle", "25 yılı aşkın deneyimimizle temizlik, bakım ve bakım-onarım için size özel, entegre çözümler sunuyoruz.");
    
    const primaryCtaLabel = primarySlide?.button_text || t("hero.button_services", "Hizmetlerimizi keşfet");
    ...
    const renderBackgroundMedia = () => {
        ...
        if (hasSlideVideo) {
            const handleVideoError = (e) => {
                console.error("❌ VİDEO YÜKLEME HATASI:", e.target.error.code, e.target.error.message);
            };
